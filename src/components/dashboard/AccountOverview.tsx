...
const AccountOverview = ({
    ...
    const spendingPercentage = Math.min(100, Math.round((spendingUsed / spendingLimit) * 100));
            ...
            <Card className="w-full">
                <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between pb-2 p-4 sm:p-6">
                    ...
                    <div className="flex items-center space-x-2 mt-2 sm:mt-0">
                        ...
                        <span className="text-sm text-muted-foreground">Secure Account</span>
                    </div>
                </CardHeader>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex items-end">
                                <span className="text-2xl sm:text-3xl font-bold">{currentUser.balance}</span>
                            </div>
                            <div className="flex items-center space-x-4 mt-2">
                                <div className="flex items-center text-green-500">
                                    <ArrowUpRight className="h-4 w-4 mr-1" />
                                    <span className="text-xs sm:text-sm font-medium">{formatCurrency(transactions.incoming)}</span>
                                </div>
                                <div className="flex items-center text-red-500">
                                    <ArrowDownRight className="h-4 w-4 mr-1" />
                                    <span className="text-xs sm:text-sm font-medium">{formatCurrency(transactions.outgoing)}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                <h3 className="text-sm font-medium text-muted-foreground">Spending Limit</h3>
                                <DollarSign className="h-4 w-4 text-muted-foreground" />
                            </div>
                            <div className="flex items-end">
                                <span className="text-2xl sm:text-3xl font-bold">{formatCurrency(spendingLimit)}</span>
                            </div>
                            <div className="space-y-2 mt-2">
                                <div className="flex justify-between text-xs sm:text-sm">
                                    <span>Used: {formatCurrency(spendingUsed)}</span>
                                    <span className={spendingPercentage > 80 ? 'text-red-500' : 'text-muted-foreground'}>{spendingPercentage}%</span>
                                </div>
                                <Progress value={spendingPercentage} className={`h-2 ${spendingPercentage > 80 ? 'bg-red-200' : ''}`} />
                            </div>
                        </div>
                ...
                <CardFooter className="border-t pt-4 p-4 sm:p-6">
                    <div className="flex flex-col sm:flex-row justify-between w-full text-xs sm:text-sm gap-2">
                        <span className="text-muted-foreground">Last updated: Today at 11:30 AM</span>
